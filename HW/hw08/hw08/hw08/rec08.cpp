...
int main() {
	...
	*/
	...
	cout << "p7: " << p7 << endl;
	cout << "Evaluating p7 at " << 5 << " yields: " << p7.evaluate(5) << endl;
...
Polynomial::Polynomial(const Polynomial& rhs)
	: header(rhs.header), degree(rhs.degree) {
	Node* current = rhs.header;
	for (size_t i = 0; i < degree + 1; ++i) {
		current->next = new Node(current->coeff, current->exponent, nullptr);
